...
<?php
    ...
    $query=mysqli_query($conn, "SELECT id FROM freeboard WHERE id >$id LIMIT 1");
    ...
    $query=mysqli_query($conn,"SELECT id FROM freeboard WHERE id <$id
...
<?
    // 조회수 업데이트
    $result=mysqli_query("UPDATE freeboard SET view=view+1 WHERE id=$id",
    $conn);
